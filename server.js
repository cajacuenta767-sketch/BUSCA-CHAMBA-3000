...
function parseAddr(g) { const plain = (g("address") || "").trim(), comp = (g("complete_address") || "").trim(); if (comp && /^[\[{]/.test(comp)) { try { const o = JSON.parse(comp); const a = [o.street, o.borough, o.city, o.state, o.country].filter(Boolean).join(", "); return { address: a || plain, city: o.city || "" }; } catch (e) {} } const a = plain && !/^[\[{]/.test(plain) ? plain : (comp && !/^[\[{]/.test(comp) ? comp : ""); const p = a.split(",").map(s => s.trim()).filter(Boolean); return { address: a, city: p[p.length - 1] || "" }; }
